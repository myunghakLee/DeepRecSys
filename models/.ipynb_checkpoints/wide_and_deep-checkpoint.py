...
# This is to decouple input queues for Wide_and_Deep network and the Wide_and_Deep network itself
# =============================================================================
class Wide_and_Deep_Wrapper(object):
    ...
    def create(self, X, S_lengths, S_indices, T):
        if self.args.queue:
            self.wnd.create(X, S_lengths, S_indices, T,
                             id_qs = self.id_qs,
                             len_qs = self.len_qs)
        else:
            self.wnd.create(X, S_lengths, S_indices, T)
    ...
    # Run the Queues to provide inputs to Wide_and_Deep model
    ...
    # =========================================================================
    # Helper  functions to build queues for Wide_and_Deep inputs (IDs, Lengths, FC)
    # in order to decouple blocking input operations
    # =========================================================================
    def build_wnd_sparse_queue(self, tag = "id", qid = None):
...
class Wide_and_Deep(object):
    ...
    def create_interactions(self, x, ly, model, tag):
        ...
        # Wide and deep concatenates sparse (cateogrical) and dense (continuous) features
    ...
    def create_model(self, X, S_lengths, S_indices, T):
        ...
        print("Trying to run Wide_and_Deep for the first time")
        if T is not None:
        ...
        print("Ran Wide_and_Deep for the first time")
...
if __name__ == "__main__":
    ...
    print("Trying to initialize Wide_and_Deep")
    with core.DeviceScope(device_opt):
        wnd = Wide_and_Deep( args )
    print("Initialized Wide_and_Deep Net")

    wnd.create(lX[0], lS_l[0], lS_i[0], lT[0])
    ...
    for k in range(args.nepochs):
        for j in range(nbatches):
            ...
            time_load_end   = wnd.run(lX[j], lS_l[j], lS_i[j], args.enable_profiling) # args.enable_profiling
